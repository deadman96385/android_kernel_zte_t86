...
#if defined(CONFIG_DRM_MSM)
static int goodix_drm_msm_state_chg_callback(struct notifier_block *nb,
...
#if defined(CONFIG_DRM_MSM)
	.notifier_call = goodix_drm_msm_state_chg_callback,
#elif defined(CONFIG_FB)
...
#endif
	if (status) {
		pr_err("%s, failed to register fb or msm_drm status=%d", __func__, status);
	}
