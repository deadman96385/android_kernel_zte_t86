...
static int silfp_fb_callback(struct notifier_block *notif, unsigned long event, void *data)
{
...
static int silfp_keyevent(struct silfp_data	*fp_dev, struct fp_dev_key_t *pkey)
{
	...
	if (fp_dev->keymap_cust.k[pkey->value - NAV_KEY_START] != KEY_RESERVED) {
		...
		input_report_key(fp_dev->input, fp_dev->keymap_cust.k[pkey->value - NAV_KEY_START],
			NAV_KEY_FLAG_DOWN);
