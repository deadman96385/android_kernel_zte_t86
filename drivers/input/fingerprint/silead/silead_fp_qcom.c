...
#define FINGER_PINCTRL_STATE_IRQ     "spi_irq"
...
static void silfp_hw_poweron(struct silfp_data *fp_dev)
{
	...
	if (fp_dev->avdd_port > 0) {
		...
		LOG_MSG_DEBUG(INFO_LOG, "%s: enable avdd_port power supply ret:%d\n", __func__, err);
	}
...
static int silfp_parse_dts(struct silfp_data *fp_dev)
{
	...
	fp_dev->pin.irq = pinctrl_lookup_state(fp_dev->pin.pinctrl, FINGER_PINCTRL_STATE_IRQ);
	if (IS_ERR_OR_NULL(fp_dev->pin.irq)) {
		fp_dev->pin.active = NULL;
		fp_dev->pin.sleep = NULL;
		fp_dev->pin.pinctrl = NULL;
		LOG_MSG_DEBUG(ERR_LOG, "%s: Failed to get pinctrl state irq\n", __func__);
		return PTR_ERR(fp_dev->pin.irq);
	}
#ifndef QSEE_V4
	/* Get iface_clk info */
...
static int silfp_resource_init(struct silfp_data *fp_dev, struct fp_dev_init_t *dev_info)
{
	...
	if (fp_dev->avdd_port > 0) {
		...
		if (ret < 0) {
			...
			LOG_MSG_DEBUG(DBG_LOG, "[%s] Success to request GPIO=%d, ret=%d",
				__func__, (s32)fp_dev->avdd_port, ret);
		}
	}
	...
	if (fp_dev->vddio_port > 0) {
		...
		if (ret < 0) {
			LOG_MSG_DEBUG(ERR_LOG, "[%s] Failed to request GPIO=%d, ret=%d",
				__func__, (s32)fp_dev->vddio_port, ret);
			status = -ENODEV;
			goto err_vddio;
		}
	}
	...
	if (ret < 0) {
		...
		ret  = request_irq(fp_dev->irq,
				   ...
				   fp_dev);
		if (ret < 0) {
			LOG_MSG_DEBUG(ERR_LOG, "[%s] Filed to request_irq (%d), ert=%d",
	...
	if (dev_info && fp_dev->pin.qup_id && (fp_dev->pin.qup_id <= 32)) {
		...
		strlcpy(dev_info->ta, TANAME, sizeof(dev_info->ta));
	}
